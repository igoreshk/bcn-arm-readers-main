...
/**
 * Test for Building Service URLs in feign clients and controllers.
 */
...
public class BuildingServiceUrlsTest {

    private static final Logger LOGGER = LogManager.getLogger(BuildingServiceUrlsTest.class);
    ...
    private final List<Class<? extends Annotation>> seekingClassAnnotations = Arrays
            .asList(FeignClient.class, RequestMapping.class);
    private final List<Class<? extends Annotation>> seekingMethodAnnotations = Arrays
            .asList(RequestMapping.class, GetMapping.class, PostMapping.class, DeleteMapping.class, PutMapping.class,
                    PatchMapping.class
            );
    private final List<Class<? extends Annotation>> seekingParamAnnotation = Collections
            .singletonList(PathVariable.class);
    ...
    private Map<String, Object> getBeansMapWithNeededAnnotation() {
        ...
        for (Class<? extends Annotation> annotation : seekingClassAnnotations) {
            beansMap.putAll(applicationContext.getBeansWithAnnotation(annotation));
        }
        return beansMap;
    }
        ...
        for (Parameter parameter : method.getParameters()) {
            for (Annotation annotation : parameter.getAnnotations()) {
                if (seekingParamAnnotation.contains(annotation.annotationType())) {
                    parameterPathVariablesList
                            .add((String) annotation.getClass().getMethod("value").invoke(annotation));
                }
            }
        }
    }
    ...
    private Optional<Class<?>> loadClass(String string) {
        try {
            return Optional.ofNullable(getClass().getClassLoader().loadClass(string));
        } catch (ClassNotFoundException e) {
            LOGGER.error("An error occurred while loading class " + string, e);
            Assert.fail();
        }
